...
/*1_Obtener los datos completos de los empleados*/
SELECT * FROM empleados;
/*2_Obtener los datos completos de los departamentos*/
SELECT * FROM departamentos;
/*3_Listar el nombre de los departamentos*/
SELECT nombre_depto FROM departamentos;
/*4_Obtener el nombre y salario de todos los empleados*/
SELECT nombre,sal_emp FROM empleados;
/*5_Listar todas las comisiones*/
SELECT comision_emp FROM empleados;
/*6_Obtener los datos de los empleados cuyo cargo sea 'Secretaria'*/
SELECT * FROM empleados WHERE cargo_emp = 'Secretaria';
/*7_Obtener los datos de los empleados venderores,ordenados por nombre alfabeticamente*/
SELECT * FROM empleados WHERE cargo_emp LIKE 'Vendedor' ORDER BY nombre ASC;
/*8_Obtener el nombre y cargo de todos los empleados, ordenados por salario de menor a mayor*/
SELECT nombre,cargo_emp FROM empleados ORDER BY sal_emp ASC;
/*9_Obtener el nombre de o de los jefes que tengan su departamento situado en la ciudad de "Ciudad real"*/
SELECT nombre_jefe_depto FROM departamentos WHERE ciudad LIKE 'Ciudad Real';
/*10_Elabore un listado donde para cada fila, figure el alias 'Nombre' y 'Cargo' para las respectivas tablas de empleados.*/
SELECT nombre AS Nombre, cargo_emp AS Cargo
FROM empleados; 
/*11_Listar los salarios y comisiones de los empleados del departamento 2000, ordenado por comision de menor a mayor*/
SELECT sal_emp, comision_emp FROM personal.empleados WHERE id_depto = 2000 ORDER BY comision_emp;
/*12_Obtener el valor total a pagar a cada empleado del departamento 3000, que resulta
de: sumar el salario y la comisión, más una bonificación de 500. Mostrar el nombre del
empleado y el total a pagar, en orden alfabético.*/
SELECT nombre, (sal_emp + IFNULL(comision_emp, 0) + 500) AS 'Total a pagar' FROM personal.empleados WHERE id_depto = 3000 ORDER BY nombre;
/*13_Muestra los empleados cuyo nombre empiece con la letra J.*/
SELECT * FROM personal.empleados WHERE nombre LIKE 'j%';
/*14_Listar el salario, la comisión, el salario total (salario + comisión) y nombre, de aquellos
empleados que tienen comisión superior a 1000.*/
SELECT sal_emp, comision_emp, (sal_emp + comision_emp) AS 'Salario Total', nombre FROM personal.empleados WHERE comision_emp > 1000;
/*15_Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen
comisión.*/
SELECT sal_emp, comision_emp, (sal_emp + comision_emp) AS 'Salario Total', nombre FROM personal.empleados WHERE comision_emp IS NULL;
/*Modifique nombres de columnas para facilitar codeo*/
/*SELECT sal_emp AS salario, comision_emp AS comision FROM personal.empleados;
/*16_Obtener la lista de los empleados que ganan una comisión superior a su sueldo.*/
SELECT * FROM personal.empleados WHERE comision_emp > sal_emp;
/*17_Listar los empleados cuya comisión es menor o igual que el 30% de su sueldo.*/
SELECT * FROM personal.empleados WHERE comision_emp <= 0.3 * sal_emp;
/*18_Hallar los empleados cuyo nombre no contiene la cadena “MA”*/
SELECT * FROM personal.empleados WHERE nombre LIKE '%MA%';
/*19_ Obtener los nombres de los departamentos que sean “Ventas”, “Investigación” o
‘Mantenimiento.*/
SELECT nombre_depto FROM personal.departamentos WHERE nombre_depto IN ('Ventas', 'Investigacion', 'Mantenimiento');
/*20_ Ahora obtener el contrario, los nombres de los departamentos que no sean “Ventas” ni
“Investigación” ni ‘Mantenimiento.*/
SELECT nombre_depto FROM personal.departamentos WHERE nombre_depto NOT IN ('VENTAS', 'INVESTIGACION', 'MANTENIMIENTO');
/*21_Mostrar el salario más alto de la empresa.*/
SELECT MAX(sal_emp) AS 'Salario mas alto' FROM personal.empleados;
/*22_Mostrar el nombre del último empleado de la lista por orden alfabético.*/
SELECT nombre FROM personal.empleados ORDER BY nombre DESC LIMIT 1;
/*23_Hallar el salario más alto, el más bajo y la diferencia entre ellos.*/
...
/*24_Hallar el salario promedio por departamento.*/
SELECT id_depto, AVG(sal_emp) AS 'Salario promedio' FROM personal.empleados GROUP BY id_depto;
/*25_Hallar los departamentos que tienen más de tres empleados. Mostrar el número de empleados de esos departamentos.*/
SELECT id_depto, COUNT(id_emp) AS 'Numero de empleados' FROM personal.empleados GROUP BY id_depto HAVING COUNT(id_emp) > 3;
/*26_Hallar los departamentos que no tienen empleados*/
SELECT id_depto, COUNT(id_emp) AS 'Numero de empleados' FROM personal.empleados GROUP BY id_depto HAVING COUNT(id_emp) IS NULL;
/*27_Mostrar la lista de empleados, con su respectivo departamento y el jefe de cada departamento.*/
...
/*28_Mostrar la lista de los empleados cuyo salario es mayor o igual que el promedio de la
